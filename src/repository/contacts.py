...
async def get_contact(contact_id: int, user: User, db: Session) -> Contact:
    return db.query(Contact).filter(and_(User.id == user.id, Contact.id == contact_id)).first()

async def get_contacts_by_fname(first_name: str, user: User, db: Session) -> List[Contact]:
    return db.query(Contact).filter(and_(User.id == user.id, Contact.first_name.like(f'%{first_name}%'))).all()

async def get_contacts_by_lname(last_name: str, user: User, db: Session) -> List[Contact]:
    return db.query(Contact).filter(and_(User.id == user.id, Contact.last_name.like(f'%{last_name}%'))).all()

async def get_contacts_by_email(email: str, user: User, db: Session) -> List[Contact]:
    return db.query(Contact).filter(and_(User.id == user.id, Contact.email.like(f'%{email}%'))).all()

async def get_contacts_by_birthday(user: User, db: Session) -> List[Contact]:
    today = datetime.today().date()
    next_week = today + timedelta(days=7)

    contacts = db.query(Contact).filter(and_(User.id == user.id, 
...
async def update_contact(note_id: int, body: ContactUpdate, user: User, db: Session) -> Contact | None:
    contact = db.query(Contact).filter(and_(User.id == user.id, Contact.id == note_id)).first()
    if contact:
...
async def remove_contact(contact_id: int, user: User, db: Session) -> Contact | None:
    contact = db.query(Contact).filter(and_(User.id == user.id, Contact.id == contact_id)).first()
    if contact:
        db.delete(contact)
        db.commit()
    return contact
